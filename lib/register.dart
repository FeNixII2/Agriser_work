...
class _RegisterState extends State<Register> {
  ...
  Future getAllprovince() async {
    // print("เข้าแล้วเน้อ");
    var url = "http://192.168.1.8/Agriser_work/getProvince.php?isAdd=true";
  ...
  Future getSelectAmphures() async {
    // print("มาอำเภอ");
    var url =
        "http://192.168.1.8/Agriser_work/getSelectAmphures.php?isAdd=true&&idprovince=$selectProvince";
    var response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      var jsonData = json.decode(response.body);
      setState(() {
  ...
  void registhread() async {
    ...
    var dio = Dio();
    final response = await dio.get(
        "http://192.168.1.8/agriser_work/addUser.php?isAdd=true&tel=$phone_user&pass=$password_user&name=$name_user&date=$formattedDate&sex=$sex_user&address=$address_user&province=$selectProvince&amphures=$selectAmphure&email=$email_user");
    print(response.data);
